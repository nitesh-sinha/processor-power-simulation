...
enum Op_code_num{
 ...
 // OP_DUMMY,		// Dummy OP for TLB insertion in MSHR
  NUM_OP_TYPE
};
...
typedef struct Trace_op_struct{
  ...
  int8_t  dst;       /* destination register id */ 
  ...
  bool write_flag;    /* Does instruction write flag (conditional code)? */
...
typedef struct Op_struct : public Trace_op{
  ...
  uint32_t  op_pool_id; 
  bool valid; 
  bool last_op;//to denote if the instruction is the last one in the trace 
  bool dummy;
